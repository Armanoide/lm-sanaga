...
use log::{debug, error};
use crate::generator::generated_token_info::{GeneratedTokenInfo, TokenStatsExt};
use rayon::prelude::*;
use std::thread;
...
pub struct StreamResponse {
    ...
    stop: bool,
    prompt_len: usize,
    responses: Vec<GeneratedTokenInfo>,
    token_receiver: Option<Receiver<GeneratedTokenInfo>>,
}

impl StreamResponse {
    pub fn new(model: Arc<RwLock<ModelKind>>, tokenizer: Rc<Tokenizer>) -> StreamResponse {
        StreamResponse {
            model,
            tokenizer,
    ...
    fn prelude_generate_text(&mut self, prompt: Vec<u32>) -> Result<()>{
        ...
        let (tx, rx): (Sender<GeneratedTokenInfo>, Receiver<GeneratedTokenInfo>) = bounded(100);
    ...
    pub fn generate_text(&mut self, prompt: Vec<u32>) -> Result<()> {
        ...
        if let Some(_generator) = &mut self.token_generator {
            ...
            let rx = self.token_receiver.take()
            ...
            for mut gti in rx {
                ...
                self.tokenizer.decode_response_from_generated_token_info(&mut gti);
        ...
        match self.responses.tokens_stats(false) {
            res_stat => {
                if let Some(stats) = res_stat {
                    let (avg_generation_tps, avg_prompt_tps, avg_peak_memory) = stats;
                    println!("================ Generation Summary ================");
                    println!("🧠 Average Prompt TPS     : {:>8.2} tokens/sec", avg_prompt_tps);
                    println!("🚀 Average Generation TPS : {:>8.2} tokens/sec", avg_generation_tps);
                    println!("💾 Average Peak Memory    : {:>8.2} MB", avg_peak_memory as f64 / (1024.0 * 1024.0));
                    println!("====================================================");
                }
            },
        }
