...
impl Module for AttentionLlama {
    fn forward(&mut self, x: &Array, mask: Option<&AttentionMask>, cache: Option<SNCacheItem>) -> Result<Array> {
        ...
        queries = queries.reshape(&[b, l, self.n_heads, -1])?.transpose_axes(&[0, 2, 1, 3])?;
        keys = keys.reshape(&[b, l, self.n_kv_heads, -1])?.transpose_axes(&[0, 2, 1, 3])?;
        values = values.reshape(&[b, l, self.n_kv_heads, -1])?.transpose_axes(&[0, 2, 1, 3])?;
        ...
        if let Some(ref mut cache_ref) = maybe_cache_ref {
            ...
            let (k, v) = cache_ref.write_lock(context)?.update_and_fetch(&keys, &values)?;
            keys = k.clone();
            values = v.clone();
        } else {
...
impl AttentionLlama {
    pub fn new(llama_config: Rc<LLaMAConfig>) -> Result<AttentionLlama> {
        ...
        if hidden_size % n_heads != 0 {
            return Err(Error::InvalidConfig("hidden_size must be divisible by n_heads".into()));
        }

        if n_heads % n_kv_heads != 0 {
            return Err(Error::InvalidConfig("n_heads must be divisible by n_kv_heads".into()));
        }

        let head_dim = hidden_size / n_heads;
        let scale = 1.0 / (head_dim as f64).sqrt();

        let q_proj = MaybeQuantized::new(LinearBuilder {
            input_dims: hidden_size,
            output_dims: n_heads * head_dim,
            bias: attention_bias,
        }.build()?);

        let k_proj = MaybeQuantized::new(LinearBuilder {
            input_dims: hidden_size,
            output_dims: n_kv_heads * head_dim,
            bias: attention_bias,
        }.build()?);

        let v_proj = MaybeQuantized::new(LinearBuilder {
            input_dims: hidden_size,
            output_dims: n_kv_heads * head_dim,
            bias: attention_bias,
        }.build()?);

        let o_proj = MaybeQuantized::new(LinearBuilder {
