...
use mlx_rs::nn::{silu, Linear, LinearBuilder};
...
impl Quantize for MLPLlama {
    fn quantize(&mut self, group_size: i32, bits: i32) -> Result<()>{
        self.gate_proj = self.gate_proj.clone().try_into_quantized(group_size, bits)?;
        self.down_proj = self.down_proj.clone().try_into_quantized(group_size, bits)?;
...
impl Module for MLPLlama {
    fn forward(&mut self, x: &Array, _: Option<&AttentionMask>, _: Option<SNCacheItem>) -> Result<Array> {
...
impl MLPLlama {
    pub fn new(config: Rc<LLaMAConfig>) -> Result<Self> {
        ...
        let gate_proj = MaybeQuantized::new(LinearBuilder {
            input_dims: dim,
            output_dims: hidden_dim,
            bias: mlp_bias,
        }.build()?);

        let down_proj = MaybeQuantized::new(LinearBuilder {
            input_dims: hidden_dim,
            output_dims: dim,
            bias: mlp_bias,
        }.build()?);

        let up_proj = MaybeQuantized::new(LinearBuilder {
            input_dims: dim,
            output_dims: hidden_dim,
            bias: mlp_bias,
        }.build()?);
