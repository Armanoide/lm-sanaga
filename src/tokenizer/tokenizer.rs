...
impl Tokenizer {
    ...
    pub fn apply_chat_template(&self, conversation: &Conversation) -> Result<(Vec<String>, Option<Vec<(usize, usize)>>)> {
        ...
        render_chat_template(conversation, None, None, &env, chat_template_name, false, false)
    }
        ...
        match self.tool.encode_batch(messages, false) {
            Ok(encoding) => {
                Ok(
                    encoding
                        .par_iter()
                        .fold(|| Vec::<u32>::new(), |a, b| {
                            [&a[..], &b.get_ids()[..]].concat()
                        })
                        .reduce(|| Vec::<u32>::new(), |a, b| {
                            [&a[..], &b[..]].concat()
                        })
                )
            },
    ...
    pub fn decode_response_from_generated_token_info(&self, generated_token_info: &mut GeneratedTokenInfo) {
    ...
    pub fn eot_ids(&self) -> Vec<u32> {
        self.tool.get_added_tokens_decoder()
            .par_iter()
            .filter_map(|(id, ad)| {
                if ad.content.contains("<|eot_id|>") || ad.content.contains("<|eos|>") || ad.content.contains("<|endoftext|>") {
                    Some(*id)
                } else {
                    None
                }
